...
export class LanguageComponent implements OnDestroy {
  ...
  constructor(private data: DataService, private spinner: NgxSpinnerService) {
    ...
    this.data.changeReportName('Language');
    this.data.changeNav('Language');
    const storeId = sessionStorage.getItem('storekeyId');
    this.getGraph(storeId);
  }
  ...
  getGraph(storeId) {
    ...
    this.pouchdb.get(storeId).then((res) => {
      ...
      const graphData = this.groupBy(userData, 'language');
      ...
      Object.keys(graphData).map((gd) => {
        let ga = graphData[gd];
        graphArr.push({ language: ga[0].language, count: ga.length });
      });
      ...
      graphArr.forEach((g) => {
        xAxis.push(g.language);
        yAxis.push(g.count);
        pieDt.push({ name: g.language, value: g.count, label: g.language });
      });
