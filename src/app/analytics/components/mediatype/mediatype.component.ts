...
export class MediatypeComponent implements OnDestroy {
  pouchdb: any;
  ...
  constructor(private data: DataService, private spinner: NgxSpinnerService) {
    ...
    this.data.changeReportName('Total Media Type');
    this.data.changeNav('MediaType');
  ...
  getGraph(storeId) {
    ...
    this.pouchdb.get(storeId).then((res) => {
      ...
      const graphData = this.groupBy(userData, 'mediatype');
      ...
      Object.keys(graphData).map((gd) => {
        let ga = graphData[gd];
        graphArr.push({ mediatype: ga[0].mediatype, count: ga.length });
      });
      ...
      graphArr.forEach((g) => {
        xAxis.push(g.mediatype);
        yAxis.push(g.count);
        pieDt.push({ name: g.mediatype, value: g.count, label: g.mediatype });
      });
