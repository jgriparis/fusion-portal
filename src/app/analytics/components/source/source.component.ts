...
export class SourceComponent implements OnDestroy, OnInit {
  ...
  constructor(private data: DataService, private spinner: NgxSpinnerService) {
    ...
    this.data.changeReportName('Top Sources');
    this.data.changeNav('Source');
  ...
  getGraph(storeId, topRecords: number) {
    ...
    this.pouchdb.get(storeId).then((res) => {
      ...
      const graphData = this.groupBy(userData, 'source');
      ...
      Object.keys(graphData).map((gd) => {
        let ga = graphData[gd];
        graphArr.push({ source: ga[0].source, count: ga.length });
      });
      ...
      graphArr.slice(0, topx).forEach((g) => {
        xAxis.push(g.source);
        yAxis.push(g.count);
        pieDt.push({ name: g.source, value: g.count, label: g.source });
      });
