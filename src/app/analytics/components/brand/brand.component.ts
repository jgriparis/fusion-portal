...
export class BrandComponent implements OnDestroy {
  ...
  constructor(private data: DataService, private spinner: NgxSpinnerService) {
    ...
    this.data.changeReportName('Brand');
    this.data.changeNav('Brand');
  ...
  getGraph(storeId, topRecords: number) {
    ...
    this.pouchdb.get(storeId).then((res) => {
      ...
      const graphData = this.groupBy(userData, 'brand');
      ...
      Object.keys(graphData).map((gd) => {
        let ga = graphData[gd];
        graphArr.push({ brand: ga[0].brand, count: ga.length });
      });
      ...
      graphArr.slice(0, topx).forEach((g) => {
        xAxis.push(g.brand);
        yAxis.push(g.count);
        pieDt.push({ name: g.brand, value: g.count, label: g.brand });
      });
