...
@Component({
  selector: 'app-lsmsem',
  templateUrl: './lsmsem.component.html',
  styleUrls: ['./lsmsem.component.css']
})
export class LsmsemComponent implements OnDestroy {
  pouchdb: any;
  ...
  constructor(private data: DataService, private spinner: NgxSpinnerService) {
    ...
    this.data.changeReportName('LSM / SEM');
    this.data.changeNav('LSMSEM');
  ...
  getGraph(storeId) {
    ...
    this.pouchdb.get(storeId).then((res) => {
      ...
      const graphData = this.groupBy(userData, 'lsm');
      ...
      Object.keys(graphData).map((gd) => {
        let ga = graphData[gd];
        graphArr.push({ lsm: ga[0].lsm, count: ga.length });
      });
      ...
      graphArr.forEach((g) => {
        xAxis.push(g.lsm);
        yAxis.push(g.count);
        pieDt.push({ name: g.lsm, value: g.count, label: g.lsm });
      });
